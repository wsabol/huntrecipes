					...
					<tr class="<?=( $Ingr[$i]['incomplete_flag'] == 1 ? "warning" : ( $Ingr[$i]['duplicate_flag'] == 1 ? "danger" : ( $Ingr[$i]['recipe_count'] == 0 ? "info" : "" ) ) )?>" >
						...
						<td>
							<label for="combine<?=$Ingr[$i]['id']?>">
								<input type="checkbox" id="combine<?=$Ingr[$i]['id']?>" class="combine-check" data-id="<?=$Ingr[$i]['id']?>" value=1 >
							</label>
						</td>
						<td>
							<span class="btn btn-sm btn-primary btnIngrEdit" data-id="<?=$Ingr[$i]['id']?>" ><i class="fa fa-edit"></i></span>
							<span class="btn btn-sm btn-success btnIngrView" data-id="<?=$Ingr[$i]['id']?>" ><i class="fa fa-search"></i></span>
						</td>
					</tr><?php
				}
				?>
