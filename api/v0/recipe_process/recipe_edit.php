...
<datalist id="ingrUnitList">
  ...
  ";
    ...
    ?>
    <option class="<?= $row['name'] . " " . $row['abbr'] . " " . $row['abbr_alt'] ?>" value="<?= $row['id'] ?>"
            data-name="<?= $row['name'] ?>"><?php
        }
        $results->free();
        ?>
</datalist>
            ...
            <div class="one-third">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="rTitle"
                           value="<?= htmlspecialchars(@$Recipe['title']) ?>" placeholder="Title">
                </div>
            </div>
            <div class="one-sixth">
                <div class="form-group">
                    <label for="rChef">Chef</label>
                    <input type="text" class="form-control" id="rChef" value="<?= @$Recipe['chef'] ?>" list="ChefList"
                           placeholder="Name">
                </div>
            </div>
            <div class="one-sixth">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="rType" value="<?= @$Recipe['type'] ?>"
                           list="RecipeTypeList" placeholder="Cookies">
                </div>
            </div>
            <div class="one-sixth">
                <div class="form-group">
                    ...
                    <select class="form-control" id="rCourse">
                        ...
                        while ($row = $results->fetch_assoc()) {
                            ?>
                            <option
                            value="<?= $row['id'] ?>" <?= ($Recipe['course_id'] == $row['id'] ? "selected" : "") ?> ><?= $row['name'] ?></option><?php
                        }
                        $results->free();
                        ?>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="rCuisine">
                        ...
                        while ($row = $results->fetch_assoc()) {
                            ?>
                            <option
                            value="<?= $row['id'] ?>" <?= ($Recipe['cuisine_id'] == $row['id'] ? "selected" : "") ?> ><?= $row['name'] ?></option><?php
                        }
                        $results->free();
                        ?>
                    </select>
        ...
        <div class="row">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="rParent">
                        ...
                        while ($row = $results->fetch_assoc()) {
                            ?>
                            <option
                            value="<?= $row['id'] ?>" <?= ($Recipe['parent_recipe_id'] == $row['id'] ? "selected" : "") ?> ><?= htmlspecialchars($row['title']) ?></option><?php
                        }
                        $results->free();
                        ?>
                    </select>
            ...
            <div class="one-sixth">
                <div class="form-group">
                    ...
                    $fAmount = "";
                    ?>
                    <input type="text" id="rServingCount" name="rServingCount" value="<?= $val_formatted ?>"
                           class="form-control">
                </div>
            </div>
            <div class="one-sixth">
                <div class="form-group">
                    ...
                    <select id="rServingUnit" name="rServingUnit" class="form-control">
                        ...
                        for ($i = 0; $i < count($Measure); $i++) {
                            ?>
                            <option
                            value="<?= $Measure[$i]['id'] ?>" <?= ($Recipe['serving_measure_id'] == $Measure[$i]['id'] ? "selected" : "") ?> ><?= $Measure[$i]['name'] ?></option><?php
                        }
                        ?>
                    </select>
                </div>
                ...
                <table id="ingrTable" class="ctable table">
                    ...
                    for ($i = 0; $i < count($RecipeIngredients); $i++) {
                        ...
                        <tr id="row<?= ($i + 1) ?>" class="ingrRow">
                            <td class="tdIngrAmount"><input placeholder="Amt" type="text"
                                                            class="form-control ingrAmount"
                                                            value="<?= $val_formatted ?>"></td>
                            <td class="tdIngrUnit">
                                <select class="form-control ingrUnit">
                                    ...
                                    for ($j = 0; $j < count($Measure); $j++) {
                                        ?>
                                        <option
                                        value="<?= $Measure[$j]['id'] ?>" <?= ($RecipeIngredients[$i]['measure_id'] == $Measure[$j]['id'] ? "selected" : "") ?> ><?= $Measure[$j]['name'] ?></option><?php
                                    }
                                    ?>
                                </select>
                            </td>
                            <td><input placeholder="Ingredient" type="text" class="form-control ingrName"
                                       value="<?= htmlspecialchars($RecipeIngredients[$i]['raw_ingredient_name']) ?>"
                                       list="IngredientsList"></td>
                            <td><input placeholder="Prep" type="text" class="form-control ingrPrep"
                                       value="<?= htmlspecialchars($RecipeIngredients[$i]['ingredient_prep']) ?>"></td>
                            <td style="padding-left: 12px; margin-bottom: 0px;"><label><input type="checkbox"
                                                                                              class="ingrOpt"
                                                                                              value="1" <?= ($RecipeIngredients[$i]['optional_flag'] == 1 ? "checked" : "") ?> >Opt</label>
                            </td>
                            <td><span class="btn btn-primary buttonInsertIngr"><i class="fa fa-plus"></i></span> <span
                                    class="btn btn-danger buttonRemoveIngr"><i class="fa fa-remove"></i></span></td>
                        </tr>
                        <?php
                    }
                    $fAmount = "";
        ...
        <div class="row">
            <div class="three-fourth">
                <div class="form-group">
                    ...
                    <textarea id="rInstr" class="form-control"
                              placeholder="Instructions"><?= $Recipe['instructions'] ?></textarea>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        function recipeSubmit(file_name) {
            ...
            $.ajax({
                url: '/API/v0/recipe_process/ajax-json/recipe_submit.json.php',
                type: 'GET',
                data: oRecipe,
