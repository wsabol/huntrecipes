...
class Course extends Common_Object {
    private SqlController $conn;
    public int $id;
    public string $name;
    public string $icon;

    public function __construct(int $course_id, SqlController $conn) {
        $this->id = $course_id;
        $this->conn = $conn;
    ...
    protected function update_from_db(): void {
        $sel_query = "select * from Course where id = {$this->id}";
    ...
    public function save_to_db(): bool {
        ...
        if ($updResult === false) {
            throw new SqlException('Error saving Course: ' . $this->conn->last_message());
        }
    ...
    public function delete_from_db(): bool {
        ...
        if ($result === false) {
            throw new SqlException('Error deleting Course: ' . $this->conn->last_message());
        }
        return true;
    }
}
