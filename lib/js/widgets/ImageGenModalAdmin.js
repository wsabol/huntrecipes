...
class ImageGenModalAdmin {
    ...
    generatePhoto() {
        ...
        if (typeof this.postData === 'function') {
            payload = this.postData.call(this)
        } else {
    ...
    show(title = '') {
        ...
        let $title = $(this.mdl_image_gen._element).find('.modal-title')
        if (!title) {
            $title.text('Generate Recipe Photo')
        } else {
            $title.text(title)
        }
...
export default ImageGenModalAdmin;
