...
import ImageGenModalAdmin from '../widgets/ImageGenModalAdmin';
...
export default function() {
    ...
    let image_gen = new ImageGenModalAdmin('#mdl-image-gen', function(selectedPhoto) {
        console.log('use photo ',  selectedPhoto.path)

        let payload = {
            action: 'set-image-filename',
            recipe_id: recipe_id,
        ...
        // this := image_gen
        let mdl = this;
        mdl.usePhotoButton.addClass('disabled')

        // Upload the image
        axios.patch('/api/v1/recipe/', payload)
            .then(response => {
                response = response.data
                console.log(response)
                $('#recipe-image').attr('src', response.data.image_filename)
                $('#recipe-image').parent().removeClass('d-none')
                mdl.hide()
            })
            .catch(error => {
                console.log('Error patching recipe with image photo: ' + error.response.data.message);
                mdl.setErrorMessage(error.response.data.message)
                mdl.usePhotoButton.removeClass('disabled')
            })
    ...
    function init() {
        ...
        // photo upload
        $('.btn-ai-recipe-image').click(function(){
            image_gen.show('Generate Recipe Photo - ' + $('.page-title[data-recipe-id]').text().trim())
        })
        ...
        $('.btn-remove-photo').click(function(){
            ...
            axios.delete('/api/v1/recipe/photo.php?recipe_id=' + recipe_id)
                .then(response => {
                    ...
                    $('#recipe-image').attr('src', response.data.image_filename)
                    $('#recipe-image').parent().addClass('d-none')
                    photo_upload.hide()
                })
                .catch(error => {
                    $('.btn-remove-photo').removeClass('disabled')
