...
export default function() {
    ...
    let photo_upload = new PhotoUploadModal(
        '#photo-upload-modal',
        '/api/v1/account/user/photo.php',
        {
            data_label: 'profile_photo',
            file_name: 'profile_' + HuntRecipes.current_user_id() + '.jpg',
            callback: function(){
                $('.btn-new-picture').removeClass('disabled')
                $('#profile-picture').attr('src', data.image_filename)
                this.hide()
            }
        }
    )
    ...
    function init() {
        ...
        $('.btn-new-picture').click(function(){
            $('.btn-new-picture').addClass('disabled')
            photo_upload.show()
        })
    }
